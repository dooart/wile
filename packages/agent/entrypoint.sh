...
if [ "${BERSERK_TEST:-}" = "true" ]; then
  TEST_REPO="${BERSERK_TEST_REPO_PATH:-/home/berserk/workspace/repo}"

  if [ ! -d "$TEST_REPO" ]; then
    echo "ERROR: BERSERK_TEST_REPO_PATH does not exist: $TEST_REPO"
    exit 1
  fi
  ...
  if [ ! -f ".berserk/prd.json" ]; then
    echo "ERROR: .berserk/prd.json not found in test repo"
    exit 1
  fi
...
const prdPath = path.join('.berserk', 'prd.json');
const progressPath = path.join('.berserk', 'progress.txt');
...
if (!fs.existsSync(progressPath)) {
  fs.writeFileSync(progressPath, '# Berserk Progress Log\n\n## Codebase Patterns\n(Autogenerated in test mode)\n\n---\n');
}

const date = new Date().toISOString().split('T')[0];
const entry = `\n---\n\n## ${date} - ${story.id}: ${story.title}\n\n**Implemented:**\n- Mocked test mode completion\n\n**Files changed:**\n- .berserk/prd.json\n- .berserk/progress.txt\n\n**Learnings:**\n- Test mode mock run\n`;
