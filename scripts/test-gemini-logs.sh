...
TMP_ROOT=$(mktemp -d /tmp/wile-gemini-logs-XXXXXX)
...
echo "# Wile Gemini logs test" > README.md
...
git add -A
git commit -m "chore: seed gemini log test" >/dev/null
git push origin main >/dev/null

cp "$ENV_TEST" .wile/secrets/.env
if grep -q "^WILE_MOCK_GEMINI=" .wile/secrets/.env; then
  sed -i '' "s/^WILE_MOCK_GEMINI=.*/WILE_MOCK_GEMINI=true/" .wile/secrets/.env
else
  echo "WILE_MOCK_GEMINI=true" >> .wile/secrets/.env
fi
...
if grep -q "^CODING_AGENT=" .wile/secrets/.env; then
  sed -i '' "s/^CODING_AGENT=.*/CODING_AGENT=GC/" .wile/secrets/.env
else
  echo "CODING_AGENT=GC" >> .wile/secrets/.env
fi
