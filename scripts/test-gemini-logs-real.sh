...
TEST_BRANCH="integration-test-gemini"
...
if [ -z "${GEMINI_OAUTH_CREDS_B64:-}" ] && [ -z "${GEMINI_API_KEY:-}" ]; then
  echo "error: GEMINI_OAUTH_CREDS_B64 or GEMINI_API_KEY must be set in .env.test" >&2
  exit 1
fi
...
TMP_ROOT=$(mktemp -d /tmp/wile-gemini-logs-real-XXXXXX)
...
echo "# Wile Gemini logs real test" > README.md
...
git add -A
git commit -m "chore: seed real gemini log test" >/dev/null
...
if grep -q "^WILE_MOCK_GEMINI=" "$RUN_DIR/.wile/secrets/.env"; then
  sed -i '' "s/^WILE_MOCK_GEMINI=.*/WILE_MOCK_GEMINI=false/" "$RUN_DIR/.wile/secrets/.env"
fi
...
if grep -q "^CODING_AGENT=" "$RUN_DIR/.wile/secrets/.env"; then
  sed -i '' "s/^CODING_AGENT=.*/CODING_AGENT=GC/" "$RUN_DIR/.wile/secrets/.env"
else
  echo "CODING_AGENT=GC" >> "$RUN_DIR/.wile/secrets/.env"
fi
