...
if [ -z "${CX_API_KEY:-}" ]; then
  echo "error: CX_API_KEY must be set in .env.test for Codex integration test" >&2
  exit 1
fi
  ...
  "userStories": [
    {
      ...
      "acceptanceCriteria": [
        "Create integration-log-codex.txt containing the text REAL CODEX LOG TEST"
      ],
      "priority": 1,
      "passes": false
    }
...
if grep -q "^CX_MODEL=" "$RUN_DIR/.wile/secrets/.env"; then
  sed -i '' "s/^CX_MODEL=.*/CX_MODEL=o3/" "$RUN_DIR/.wile/secrets/.env"
else
  echo "CX_MODEL=o3" >> "$RUN_DIR/.wile/secrets/.env"
fi
